...
const getDates = (year) => {
	...
	for (const d of jsonDates) {
		...
		if (ruleInMonth.length === 2) {
			...
			// create dates for movable holidays
			let [dayOfTheWeek, dayOfTheWeekPos, dayOfTheWeek2, dayOfTheWeekPos2] = ruleInMonth;
			...
			let day2 = getDay(
				year,
				formatter(month),
				parseInt(dayOfTheWeek2),
				parseInt(dayOfTheWeekPos2)
			);
...
const formatDate = (date) =>
	date.includes("-") ? formatObjectDoubleDate(date) : formatObjectSingleDate(date);
